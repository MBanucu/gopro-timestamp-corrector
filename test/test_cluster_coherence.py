...
class TestClusterCoherence(unittest.TestCase):
    ...
    def _cluster_offset(self, boot, cluster):
        return boot['cluster_heap_offset'] + (cluster - 2) * boot['cluster_size']

    def _find_dir_cluster(self, boot, device, name, start_cluster):
        found = self._fs.find_in_dir(boot, device, start_cluster, name)
        if not found:
            return None
    ...
    def test_write_does_not_corrupt_parent_cluster(self):
        ...
        self._ops.fix_exfat_raw(first, target_dt, dry_run=False)
        ...
        for attempt in range(3):
            found = self._fs.find_in_dir(boot, dev_str, dcim_cluster, '100GOPRO')
        ...
        re_found = self._fs.find_file_entry(boot, dev_str, first)
        self.assertIsNotNone(re_found, f'Could not find {Path(first).name} after write')

        raw_mtime = self._ops.read_mtime_raw(first)
