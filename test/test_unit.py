...
class TestReadExfatMtimeRaw(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run(['cp', '--sparse=always', str(cached), str(cls._img)], check=True, capture_output=True)
        try:
            cls._loop, cls._mnt = setup_loop_device(cls._img)
    ...
    def test_reads_mtime_from_directory_entry(self):
        ...
        self.assertGreater(ts, 1778000000)  # reasonable timestamp for May 2026
    ...
    def test_mtime_matches_btime_before_correction(self):
        """Before any correction, mtime and btime should be close (within 1s)."""
        ...
        # On GoPro files, mtime and btime might differ by creation process,
        # but should be within a few seconds
        self.assertLessEqual(abs(mtime - btime), 5,
                             f'mtime={mtime} btime={btime} diff={abs(mtime-btime)}')

    def test_mtime_matches_stat_on_nixos(self):
        """On kernels where os.utime() works, stat mtime should match raw mtime."""
        from strategies.exfat_raw import read_exfat_mtime_raw
        files = sorted(self._target.glob('*.MP4'))
        try:
            os.utime(files[0], (1234567890.0, 1234567890.0))
            raw = read_exfat_mtime_raw(str(files[0]))
            # raw readback might not see the os.utime() change (bypasses kernel cache)
...
class TestExfatRawBtimeDt(unittest.TestCase):
    ...
    def test_btime_dt_preserves_creation_time(self):
        """_fix_exfat_raw with btime_dt must keep original btime."""
        ...
        # Write only mtime via _fix_exfat_raw with btime_dt preserved
        ...
        self.assertEqual(after_btime_raw, orig_btime_raw,
                         f'btime changed from {orig_btime_raw} to {after_btime_raw}')

        # Verify mtime changed via raw readback
        from strategies.exfat_raw import read_exfat_mtime_raw
        ...
        self.assertEqual(after_mtime_raw, expected_ts,
                         f'mtime ({after_mtime_raw}) != expected ({expected_ts})')
